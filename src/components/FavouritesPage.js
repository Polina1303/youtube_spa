...
function FavouritesPage({   modalActive, setModalActive }) {
    ...
    return (
        <div>
            ...
            {
                favorite.length ?
                    favorite.filter(item => user == item.login)
                        .map(item => {
                            return (
                                <div key={item.id} className='button_favourites'>
                                    <Button type="text" onClick={() => onSearchFromFavourites(item.request)}>
                                        {item.title}
                                    </Button>
                                    <div className="button_change">
                                    <Button type="link" onClick={() => onChangeFromFavorite(item)}>
                                        Изменить
                                    </Button>
                                    <Button type="text" danger onClick={() => dispatch(deleteFavourites(item.id))}>
                                        Удалить
                                    </Button>
                                    </div>
